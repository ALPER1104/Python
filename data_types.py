# Değişken alıştırması
ondalikli_sayi=10.934
print(type(ondalikli_sayi))

sayi=5
print(type(sayi))

green = True
print(green)
print(type(green))
green = 1
print(type(green))

ad = input ("Adınızı giriniz: ")
soyad = input ("Soyadınınz girininiz: ")
isim = ad+ " " + soyad
print(isim)
print(type(isim))
karakter_sayisi = len(isim)
print("İsminizin toplam karakter sayısı:",karakter_sayisi)
if karakter_sayisi<12:
    print("isminizin tooplam karakter sayısı gerekli şartı sağlamamaktadır")

else:
    print("isminizin toplam karakter sayisi gerkli şartı sağlamaktadır diğer bilgilerinizi girmeye devam edebilirsiniz")



